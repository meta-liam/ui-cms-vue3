  ...
  <div class="liam_container">
    <div class="title">
      <div class="search_box">
        <el-button type="primary" plain class="add" @click="callBack('add', {})">新加任务</el-button>
    ...
    <el-table :data="items" style="width: 100%" height="75vh">
      <el-table-column fixed prop="id" label="TaskID" width="180" />
      <el-table-column fixed prop="name" label="名称" width="120" />
      <el-table-column prop="spec" label="调度时间" width="120" />
      <el-table-column prop="created" label="建立时间" width="180" />
      <el-table-column prop="updated" label="更新时间" width="180" />
      <el-table-column prop="status" label="启动状态" width="100" />
      <el-table-column prop="isNotify" label="通知状态" width="100" />
      <el-table-column prop="doStatus" label="执行状态" width="100" />
      <el-table-column prop="workerType" label="Worker类型" width="120" />
...
import ItemEdit from './components/cron-edit'
import { getItems, addOrEditItem, setAddNew, setEditData, deleteItem } from './cron'
...
export default {
  ...
  setup() {
    ...
    const page = ref(p)
    const editIfo = ref({ editTitle: '新加任务' })
    const tags = ref([
